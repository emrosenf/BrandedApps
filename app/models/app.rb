class App < ActiveRecord::Base
  ...
  def send_all_feed_notifications
    unsentListNoty = unsent_list_notifications
    return if (unsentListNoty.length == 0)
    begin
      APN::Connection.open_for_delivery({:cert => self.cert}) do |conn, sock|
        unsentListNoty.each do |lnoty|
          Rails.logger.info "Sending List Notification ##{lnoty.id}"
          list = List.includes(:subscribers => [:subscriber_infos]).find lnoty.list_id
          subscribers = list.subscribers
          infos = subscribers.collect{|s| s.subscriber_infos}
          infos.flatten!
          infos.each do |info|
            begin
              if info.is_iphone?
                raw = lnoty.message_for_sending(info)
                if raw.length == 0 or raw == ""
                  Rails.logger.info "Something is wrong with the message"
                else 
                  conn.write(raw)
                  Rails.logger.info "Wrote to subscriber #{info.token} with message length: #{raw.length}"  
                end
